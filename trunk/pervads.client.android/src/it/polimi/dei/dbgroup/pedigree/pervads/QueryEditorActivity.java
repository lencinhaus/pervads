...
import it.polimi.dei.dbgroup.pedigree.pervads.context.ContextSearchManager;
import it.polimi.dei.dbgroup.pedigree.pervads.context.ContextSearchManager.OnValueSelectedListener;
import it.polimi.dei.dbgroup.pedigree.pervads.query.Assignment;
import it.polimi.dei.dbgroup.pedigree.pervads.query.Query;
import it.polimi.dei.dbgroup.pedigree.pervads.util.InitializationManager;
import it.polimi.dei.dbgroup.pedigree.pervads.util.InitializationManager.OnFinishListener;
...
public class QueryEditorActivity extends ListActivity {
	...
	private void checkSelectedValue(Value value) {
		...
		for (int i = 0; i < query.getAssignments().size(); i++) {
			Assignment assignment = query.getAssignments().get(i);
		...
		Assignment assignment = new Assignment(value.getURI());
		assignmentAdded(assignment);
	}

	private void replaceAssignment() {
		Assignment assignment = query.getAssignments().get(
	...
	private void editAssignment(int id) {
		editAssignmentId = id;
		Assignment assignment = query.getAssignments().get(id);
		Intent intent = new Intent(this, AssignmentNavigationActivity.class);
